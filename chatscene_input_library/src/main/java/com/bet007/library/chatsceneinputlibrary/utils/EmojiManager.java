...
public class EmojiManager {
    ...
    private static final void load(Context context, String xmlPath) {
        ...
        //补充最后一页少的表情
        int tmp = mDefaultEntries.size() % EmotiomFragment.EMOJI_PER_PAGE;
        if (tmp != 0) {
            int tmp2 = EmotiomFragment.EMOJI_PER_PAGE -
                    (mDefaultEntries.size() - (mDefaultEntries.size() / EmotiomFragment.EMOJI_PER_PAGE) * EmotiomFragment.EMOJI_PER_PAGE);
