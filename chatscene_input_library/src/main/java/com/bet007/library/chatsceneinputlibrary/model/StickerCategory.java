package com.bet007.library.chatsceneinputlibrary.model;
...
//                stickers.add(new StickerItem(name, name + File.separator + file.getName()));
        ...
        //补充最后一页缺少的贴图
        int tmp = stickers.size() % EmotiomFragment.STICKER_PER_PAGE;
        if (tmp != 0) {
            int tmp2 = EmotiomFragment.STICKER_PER_PAGE -
                    (stickers.size() - (stickers.size() / EmotiomFragment.STICKER_PER_PAGE) * EmotiomFragment.STICKER_PER_PAGE);
